...
class Ideogram4Pipeline:
  ...
  def from_pretrained(
    ...
    *,
    ...
    progress = progress_callback or (lambda _stage, _current, _total: None)
    progress("download conditional transformer", 0, 1)
    conditional_state_dict = _load_indexed_or_single_state_dict(
      config.weights_repo, config.conditional_index_filename
    )
    progress("download conditional transformer", 1, 1)
    progress("download unconditional transformer", 0, 1)
    unconditional_state_dict = _load_indexed_or_single_state_dict(
      config.weights_repo, config.unconditional_index_filename
    )
    progress("download unconditional transformer", 1, 1)
    progress("download VAE", 0, 1)
    autoencoder_weights = hf_hub_download(
      repo_id=config.weights_repo, filename=config.autoencoder_filename
    )
    progress("download VAE", 1, 1)

    progress("load conditional transformer", 0, 1)
    ...
    progress("load conditional transformer", 1, 1)
    progress("load unconditional transformer", 0, 1)
    ...
    progress("load unconditional transformer", 1, 1)

    progress("load text encoder", 0, 1)
    ...
    progress("load text encoder", 1, 1)
    progress("load VAE", 0, 1)
    autoencoder = _load_autoencoder(autoencoder_weights, device, dtype)
    progress("load VAE", 1, 1)
  ...
  def __call__(
    ...
    *,
    ...
    for i in range(num_steps - 1, -1, -1):
      ...
      delta = s_val - t_val
      z = z + v * delta
      progress("denoise", num_steps - i, num_steps)

    progress("decode VAE", 0, 1)
    images = self._decode(z, grid_h=grid_h, grid_w=grid_w)  # type: ignore[arg-type]
    progress("decode VAE", 1, 1)
    return images
